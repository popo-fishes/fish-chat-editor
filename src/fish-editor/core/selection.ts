...
class Selection {
  ...
  public deleteRange(range: IRange, cb?: () => void): void {
    ...
    // console.log(range)
    /**
     * When Firefox browser selects all, its starting node is directly edited to the root node,
     *  so the judgment of selecting all is different
     */
    if (
      range.startContainer === this.root &&
      range.endContainer === this.root &&
      range.startOffset === 0 &&
      range.endOffset === this.root.childNodes.length
    ) {
      const lineDom = base.createLineElement()
      dom.toTargetAddNodes(this.root as any, [lineDom])
      this.setCursorPosition(lineDom.firstChild, 'before')
      cb?.()
      return
    }
  ...
  public getNativeRange(): Range | null {
    const selection = this.getSelection()
  ...
  public normalizeNative(nativeRange: Range) {
    if (
